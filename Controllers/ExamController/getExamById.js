...
const getExamById = async (req, res) => {
    ...
    if (req.user.toObject().hasOwnProperty('course') && req.user.course.isPremium) {
        ...
        ExamModel.find({ $and: [{ endTime: { $gt: new Date(new Date() - (24 * 60 * 60 * 1000)) } }] }).populate(['subjectId', 'curriculumId', 'subjectId', 'moduleId', 'broadQuestionsId', 'mcqsId']).sort({ startTime: -1 }).then(data => {
